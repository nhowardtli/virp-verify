...
pub fn grade_producer_signatures(
    ...
    // signature is wrong whether or not a key was supplied.
    for (seq, body) in &cam_bodies {
        let has_kid = body.get("producer_key_id").and_then(Value::as_str).is_some();
        let has_sig = body.get("producer_sig").and_then(Value::as_str).is_some();
        if !has_kid || !has_sig {
            let missing = if has_kid { "producer_sig" } else { "producer_key_id" };
            return ProducerSignerReport {
                signature_validity: Status::failed(format!(
                    "camera record at chain sequence {seq} carries no {missing}"
                )),
                ...
                trust_source: None,
                detail: "a camera record does not carry the producer signature fields its schema \
                         requires"
                    .to_owned(),
                claimed_key_ids: claimed,
            };
    ...
    if keys.is_empty() {
        return ProducerSignerReport::unverifiable(
            "producer signature not checked: no producer public key was supplied \
             (--producer-key; the key must arrive out of band — the bundle carries each \
             record's producer_key_id, never the producer key)",
            claimed,
        );
    }
